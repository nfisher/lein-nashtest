...
(defn ^:no-project-needed nashtest
  ...
  [project & keys]
  (let [{{:keys [id load-js runner test-main]} :nashtest} project]
    (when (and (nil? runner) (nil? test-main) (nil? id))
      (lein-main/abort "nashtest: id, runner, or test-main must be specified in your project.clj."))
